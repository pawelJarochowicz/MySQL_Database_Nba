...
public class DatabaseNba {
    public static void main(String[] args)throws SQLException {
        ...
        statement.executeUpdate("CREATE TABLE if not exists player(player_id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, first_name varchar(25) not null, last_name varchar(25) not null, age int not null, position varchar(2) not null, team_id int, foreign key(team_id) references team(team_id));");
        statement.executeUpdate("CREATE TABLE if not exists team_stats(team_stats_id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, off_points int not null, def_points int not null, off_rating int not null, def_rating int not null, home_adv int not null, away_adv int not null, team_id int, foreign key(team_id) references team(team_id));");
